...
import {
  ONCHAIN_ACTION_LABELS,
  ONCHAIN_PERMISSION_PRESETS,
  ONCHAIN_VISIBILITY_LABELS,
} from "@/lib/onchain/onchain-config";
import type { ProjectOnchainPermissionAssignmentRecord } from "./types";

type PermissionPresetKey =
  | "view_only"
  | "summary_viewer"
  | "case_reviewer"
  | "project_onchain_lead";
...
const PRESET_OPTIONS = [
  ...
  ...ONCHAIN_PERMISSION_PRESETS.map((preset) => ({
    key: preset.key,
    label: preset.label,
...
function getAssignmentFromPreset(
  subjectAuthUserId: string,
  presetKey: PermissionPresetKey
): EditableAssignment {
  const preset = ONCHAIN_PERMISSION_PRESETS.find((item) => item.key === presetKey);
  if (!preset) {
    return {
      subjectAuthUserId,
      presetKey,
...
export default function ProjectOnchainPermissionsPanel({
  projectId,
  teamMembers,
  canManage,
}: {
  ...
  useEffect(() => {
    ...
    async function loadAssignments() {
      ...
      try {
        const response = await fetch(`/api/projects/${projectId}/onchain-permissions`, {
          cache: "no-store",
        });
        const payload = await response.json().catch(() => null);

        if (!response.ok || !payload?.ok) {
          throw new Error(payload?.error ?? "Failed to load on-chain permissions.");
        }
        ...
        for (const assignment of (payload.assignments ??
          []) as ProjectOnchainPermissionAssignmentRecord[]) {
          const presetKey = (
            assignment.status === "revoked"
              ? "view_only"
              : assignment.presetKey ?? "case_reviewer"
        ...
        if (active) {
          setNotice(error instanceof Error ? error.message : "Failed to load on-chain permissions.");
        }
      } finally {
        if (active) {
          setLoading(false);
  ...
  async function handleSave() {
    try {
      ...
      if (!response.ok || !payload?.ok) {
        throw new Error(payload?.error ?? "Failed to save on-chain permissions.");
      }
      ...
      setAssignments(nextAssignments);
      setNotice(payload.message ?? "On-chain permissions saved.");
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Failed to save on-chain permissions.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <OpsPanel
      ...
      title="Project on-chain permissions"
      description="Owners decide exactly who can inspect on-chain cases and who can run project-safe recovery actions. Everyone else stays summary-only by default."
      action={
        canManage ? (
          <button
            type="button"
    ...
    >
                ...
                <div className="mt-4 grid gap-4 xl:grid-cols-2">
                  <PermissionList
                    ...
                    items={assignment.visibilityPermissions.map(
                      (permission) =>
                        ONCHAIN_VISIBILITY_LABELS[
                          permission as keyof typeof ONCHAIN_VISIBILITY_LABELS
                        ] ?? permission
                    )}
                  ...
                  <PermissionList
                    ...
                    items={assignment.actionPermissions.map(
                      (permission) =>
                        ONCHAIN_ACTION_LABELS[
                          permission as keyof typeof ONCHAIN_ACTION_LABELS
                        ] ?? permission
                    )}
      ...
      {loading ? <div className="mt-4 text-sm text-sub">Loading on-chain grants...</div> : null}
    </OpsPanel>
  );
}
