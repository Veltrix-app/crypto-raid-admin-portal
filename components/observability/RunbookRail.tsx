    ...
    <div className="space-y-4 rounded-[22px] border border-line bg-[linear-gradient(180deg,rgba(13,19,29,0.96),rgba(10,15,24,0.98))] p-5 shadow-[0_16px_48px_rgba(0,0,0,0.22)]">
      ...
      <div className="grid gap-3">
        {runbooks.map((runbook) => (
          <div
            key={runbook.key}
            className="rounded-[18px] border border-line bg-card2 px-4 py-4"
          >
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <Link
                href={runbook.href}
                className="rounded-[16px] border border-primary/35 bg-primary/15 px-3.5 py-2 text-[13px] font-semibold text-primary transition hover:border-primary/50 hover:bg-primary/20"
              >
                Open surface
              </Link>
            </div>
            ...
            <div className="mt-3.5 grid gap-2">
              {runbook.checklist.map((step) => (
                <div
                  key={step}
                  className="rounded-[16px] border border-white/8 bg-black/20 px-3.5 py-2.5 text-sm leading-5.5 text-sub"
                >
                  {step}
                </div>
              ))}
