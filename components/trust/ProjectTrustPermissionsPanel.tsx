...
import {
  TRUST_ACTION_LABELS,
  TRUST_PERMISSION_PRESETS,
  TRUST_VISIBILITY_LABELS,
} from "@/lib/trust/trust-config";
import type { ProjectTrustPermissionAssignmentRecord } from "./types";

type PermissionPresetKey = "view_only" | "summary_viewer" | "case_reviewer" | "project_trust_lead";
...
const PRESET_OPTIONS = [
  ...
  ...TRUST_PERMISSION_PRESETS.map((preset) => ({
    key: preset.key,
    label: preset.label,
...
function getAssignmentFromPreset(subjectAuthUserId: string, presetKey: PermissionPresetKey): EditableAssignment {
  const preset = TRUST_PERMISSION_PRESETS.find((item) => item.key === presetKey);
  if (!preset) {
    return {
      subjectAuthUserId,
      presetKey,
...
export default function ProjectTrustPermissionsPanel({
  projectId,
  teamMembers,
  canManage,
}: {
  ...
  useEffect(() => {
    ...
    async function loadAssignments() {
      ...
      try {
        const response = await fetch(`/api/projects/${projectId}/trust-permissions`, {
          cache: "no-store",
        });
        const payload = await response.json().catch(() => null);

        if (!response.ok || !payload?.ok) {
          throw new Error(payload?.error ?? "Failed to load trust permissions.");
        }
        ...
        for (const assignment of (payload.assignments ?? []) as ProjectTrustPermissionAssignmentRecord[]) {
          const presetKey = (
            assignment.status === "revoked" ? "view_only" : assignment.presetKey ?? "case_reviewer"
        ...
        if (active) {
          setNotice(error instanceof Error ? error.message : "Failed to load trust permissions.");
        }
      } finally {
        if (active) {
          setLoading(false);
  ...
  async function handleSave() {
    try {
      ...
      if (!response.ok || !payload?.ok) {
        throw new Error(payload?.error ?? "Failed to save trust permissions.");
      }
      ...
      setAssignments(nextAssignments);
      setNotice(payload.message ?? "Trust permissions saved.");
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Failed to save trust permissions.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <OpsPanel
      ...
      title="Project trust permissions"
      description="Owners decide exactly who can see trust cases and who can take trust actions. Everyone else stays view-only by default."
      action={
        canManage ? (
          <button
            type="button"
    ...
    >
      {!canManage ? (
        <div className="rounded-[24px] border border-line bg-card px-5 py-6 text-sm text-sub">
          Only project owners can change trust visibility and action grants.
        </div>
      ) : (
        <div className="grid gap-4">
          ...
          {eligibleMembers.length === 0 ? (
            <div className="rounded-[24px] border border-line bg-card px-5 py-6 text-sm text-sub">
              No active teammates with linked accounts are available for trust grants yet.
            </div>
          ) : null}
                ...
                <div className="mt-4 grid gap-4 xl:grid-cols-2">
                  <PermissionList
                    ...
                    items={assignment.visibilityPermissions.map(
                      (permission) => TRUST_VISIBILITY_LABELS[permission as keyof typeof TRUST_VISIBILITY_LABELS] ?? permission
                    )}
                    fallback="View-only default"
                  />
                  <PermissionList
                    title="Actions"
                    items={assignment.actionPermissions.map(
                      (permission) => TRUST_ACTION_LABELS[permission as keyof typeof TRUST_ACTION_LABELS] ?? permission
                    )}
      ...
      {loading ? (
        <div className="mt-4 text-sm text-sub">Loading trust grants...</div>
      ) : null}
    </OpsPanel>
  );
}
