...
export default function ProjectsRosterTable({
  ...
  if (projects.length === 0) {
    return (
      <div className="rounded-[18px] border border-white/6 bg-white/[0.02] px-4 py-5 text-sm text-sub">
        {emptyState}
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-[18px] border border-white/6 bg-[linear-gradient(180deg,rgba(11,14,20,0.98),rgba(7,9,14,0.98))]">
      <div className="hidden grid-cols-[minmax(0,1.8fr)_120px_220px_140px_150px] gap-3 border-b border-white/6 px-4 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-sub lg:grid">
        <span>Workspace</span>
        <span>Chain</span>
        <span>State</span>
        <span>Coverage</span>
          ...
          <article key={project.id} className="px-4 py-3">
            <div className="hidden items-center gap-3 lg:grid lg:grid-cols-[minmax(0,1.8fr)_120px_220px_140px_150px]">
              <div className="flex min-w-0 items-center gap-3">
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[12px] border border-white/6 bg-white/[0.03] text-[0.95rem]">
                  {project.logo}
                </span>
...
function ProjectMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[14px] border border-white/6 bg-white/[0.02] px-2.5 py-2">
