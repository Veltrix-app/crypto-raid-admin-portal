  ...
  if (view === "setup") {
    ...
    return (
      <div className="space-y-4">
        {selectedStep ? (
          <div className="rounded-[28px] border border-primary/25 bg-[linear-gradient(180deg,rgba(186,255,59,0.08),rgba(11,16,24,0.98))] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-sm leading-7 text-sub">{selectedStep.summary}</p>
              </div>
              <OpsStatusPill
                tone={
            ...
            {selectedStep.blockers.length > 0 ? (
              <div className="mt-5 grid gap-3">
                {selectedStep.blockers.map((blocker) => (
                  <div
                    key={blocker}
                    className="rounded-[20px] border border-white/10 bg-black/20 px-4 py-4 text-sm leading-6 text-sub"
                  >
                    {blocker}
                  </div>
                ))}
              ...
              <div className="mt-5 rounded-[20px] border border-emerald-400/20 bg-emerald-500/10 px-4 py-4 text-sm text-emerald-200">
            ...
            <Link
              ...
              className="mt-5 inline-flex items-center gap-2 rounded-[18px] border border-primary/30 bg-primary/12 px-4 py-3 text-sm font-bold text-primary transition hover:bg-primary/18"
            >
              Open step rail
              <ArrowRight size={16} />
            </Link>
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3">
        {groups.map((group) => (
          <div
            key={group.id}
            className="rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,20,31,0.98),rgba(10,14,22,0.98))] p-5"
          >
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="max-w-2xl">
                ...
                <p className="mt-2 text-sm leading-6 text-sub">{group.summary}</p>
              </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {group.signals.map((signal) => (
                <div
                  key={signal}
                  className="rounded-[18px] border border-white/8 bg-black/20 px-4 py-3 text-sm text-sub"
                >
                  {signal}
                </div>
              ))}
...
function IssueGroup({
  ...
  return (
    <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(15,20,31,0.98),rgba(10,14,22,0.98))] p-5">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-2 text-sm leading-6 text-sub">{issue.summary}</p>
              </div>
