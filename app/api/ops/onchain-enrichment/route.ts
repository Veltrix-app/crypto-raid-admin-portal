...
export async function POST(request: NextRequest) {
  if (!communityBotUrl) {
    return NextResponse.json(
      { ok: false, error: "COMMUNITY_BOT_URL is missing for on-chain enrichment." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    const response = await fetch(
      `${communityBotUrl.replace(/\/+$/, "")}/jobs/enrich-onchain-events`,
      {
        method: "POST",
        headers: {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "On-chain enrichment failed.",
      },
      { status: 500 }
    );
  }
