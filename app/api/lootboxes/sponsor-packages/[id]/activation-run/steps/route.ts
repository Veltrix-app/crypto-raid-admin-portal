...
export async function POST(
  ...
  try {
    ...
    const stepRequest = buildLootboxSponsorActivationRunStepRequest({
      packageRow,
    ...
    if (!stepRequest.ok) {
      const status = stepRequest.error.includes("Stage an activation run") ? 409 : 400;
      return NextResponse.json({ ok: false, error: stepRequest.error }, { status });
    }
    ...
    const noteResponse = await admin.serviceSupabase
      ...
      .insert(
        buildLootboxSponsorPackageNoteInsertRow({
          ...
          adminAuthUserId: admin.authUserId,
          payload: stepRequest.notePayload,
        })
      )
    ...
    const metadataUpdate = await admin.serviceSupabase
      .from("lootbox_sponsor_packages")
      .update({
        metadata: buildLootboxSponsorActivationRunStepMetadataPatch({
    ...
    const auditResult = await admin.serviceSupabase
      ...
      .insert(
        buildLootboxSponsorPackageAuditPayload({
          ...
          sponsorPackageId: packageRow.id,
          action: stepRequest.audit.action,
          summary: stepRequest.audit.summary,
          metadata: {
            ...stepRequest.audit.metadata,
            noteId: noteResponse.data.id,
    ...
    if (auditResult.error) {
      console.error("Sponsor activation run step audit log skipped:", auditResult.error.message);
    }
    ...
    return NextResponse.json({
      ...
      warning: metadataUpdate.error
        ? `Activation run step note saved, but package metadata refresh failed: ${metadataUpdate.error.message}`
        : null,
    });
  } catch (error) {
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Sponsor activation run step failed.",
      },
      { status: 500 }
    );
  }
