...
export default function SubmissionsPage() {
  ...
  const autoApprovedCount = submissions.filter((submission) => submission.status === "approved" && !(flagsBySubmissionId.get(submission.id)?.length)).length;
  const flaggedFlowCount = submissions.filter((submission) => (flagsBySubmissionId.get(submission.id)?.length ?? 0) > 0).length;
    ...
    <AdminShell>
      <div className="space-y-6">
        <OpsHero
          ...
          title="Submissions"
          description="Track proof traffic, see which decisions are automatic and surface risky submissions before they disappear into a queue."
        />

        <div className="grid gap-4 md:grid-cols-4">
          <OpsMetricCard label="Pending" value={pendingCount} emphasis={pendingCount > 0 ? "warning" : "default"} />
          <OpsMetricCard label="Approved" value={approvedCount} emphasis={approvedCount > 0 ? "primary" : "default"} />
          <OpsMetricCard label="Rejected" value={rejectedCount} emphasis={rejectedCount > 0 ? "warning" : "default"} />
          <OpsMetricCard label="Flagged flow" value={flaggedFlowCount} emphasis={flaggedFlowCount > 0 ? "warning" : "default"} />
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          ...
          >
            <div className="grid gap-4">
              ...
              <SignalRow label="Open review flags" value={`${reviewFlags.filter((flag) => flag.status === "open" && flag.sourceTable === "quest_submissions").length} alerts`} />
        ...
        >
          <div className="overflow-hidden rounded-[24px] border border-line bg-card2">
            ...
            {filteredSubmissions.length === 0 ? (
              <div className="px-5 py-8 text-sm text-sub">No submissions match your filters.</div>
            ) : null}
          </div>
        </OpsPanel>
      </div>
